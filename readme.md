...
player.h is an example of a custom user-made class for the specific game (where you would put your code :).

ENGINE is now a global object - no need to create an instance of it. Just call `init` to start and `close` to finish.

```c++
// main.cpp
#include <iostream>
#include "../path/toENGINE.h"
#include "./path/to/player.h"
#include "./path/to/enemyCreator.h"

EnemyCreator* ec;

void addSprites() {
	ENGINE.addSprite("player", "./path/to/player.png");
	ENGINE.addSprite("enemy", "./path/to/enemy.png");
}
...
public:
  ...
  void update();
private:
	// add some user defined stuff here
