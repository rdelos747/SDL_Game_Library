...
- engine class is extended by user, eg myEngine::Engine
- Engine has override methods like update, keypressed, etc
- Engine has internal methods that orchestrate the whole thing

# ENGINE

## Base methods

Meant to handle automatic processing of engine functions, and not overridden by derrived class.

- constructor
- run (calls override funcs)
- render (draws object's sprites to screen)

## Override methods

Calling Engine.run() invokes the following methods

- update
- onKeyPressed
- onKeyReleased

Other methods that can be called whenever
...
- Returns pointer to object with collisionLayer _layer_ found at point x, y. Returns NULL if no object found.
